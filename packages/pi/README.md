...
- **Adds a slash command** (`/prompts history`) for a human-oriented summary of intent for a file.

Records live in the project (under `.prompts/` by default). Commit that folder if you want intent to travel with the code.
...
2. On a successful edit/write, the extension buffers what changed. When the turn ends, it appends one record (prompt, model, file/region pointers) to the nearest `.prompts` store.
...
Asks the agent to load provenance for that file and summarize intent for a human (why the code looks this way). The turn is explanation-only: it should not edit files, and auto-record is skipped so the summary itself does not become a new provenance entry.
...
Optional. With no config, the store is discovered by walking up from the edited or read file: prefer `dotprompts.json` or `.prompts/config.json`, else use `<gitRoot>/.prompts`, else `<cwd>/.prompts`.
